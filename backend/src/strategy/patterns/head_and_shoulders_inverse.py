...
class headAndShouldersInverse(Strategy):
    ...
    Pattern Shape:
          B     D         <- Neckline points (maxima)
         /\     /\
        A  \   /  E       <- Left shoulder, Right shoulder (minima)
            \ /
             C            <- Head (lowest minima)

    Confirmation: Price closes ABOVE neckline (neck2)
    Signal: LONG
    """

    def __init__(self, bars: DataHandler, events: Queue) -> None:
        self.name = "Inverse Head and Shoulders"
        self.datapoints = 5
        self.bias = "Long"
    ...
    def calculate_signals(self, event: Queue) -> None:
        """
        Process each MarketEvent and check for inverse head and shoulders pattern.
    ...
    def pattern_scanner(self, minima: pd.Series, maxima: pd.Series, frequency: str = 'daily') -> list:
        """
        Scan for inverse head and shoulders pattern: A-B-C-D-E where:
        - A, C, E are minima (left shoulder, head, right shoulder)
        - B, D are maxima (neckline points)
        - C < A and C < E (head is lowest)
        """
        patterns = []
        ...
        for i in range(self.datapoints, len(min_max) + 1):
            ...
            # cond_1: A, C, E are in minima (shoulders and head)
            cond_1 = all(x in minima.values for x in [A, C, E])

            # cond_2: B, D are in maxima (neckline)
            cond_2 = all(x in maxima.values for x in [B, D])

            # cond_3: Head (C) is below shoulders, shoulders are below neckline
            cond_3 = (A > C) and (E > C) and (B > A) and (D > A) and (B > E) and (D > E)
    ...
    def get_PriceData(self, data: pd.DataFrame, pattern_list: list) -> pd.DataFrame:
        ...
        pattern_data['sh1_price'] = data.loc[pattern_data.sh1_date, 'Low'].values
        pattern_data['neck1_price'] = data.loc[pattern_data.neck1_date, 'High'].values
        pattern_data['head_price'] = data.loc[pattern_data.head_date, 'Low'].values
        pattern_data['neck2_price'] = data.loc[pattern_data.neck2_date, 'High'].values
        pattern_data['sh2_price'] = data.loc[pattern_data.sh2_date, 'Low'].values
    ...
    def get_ConfDate(self, data: pd.DataFrame, pattern_data: pd.DataFrame):
        ...
        if len(pattern_data) != 0:
            ...
            if num_confirmed > 0:
                print(f"[headAndShouldersInverse] Pattern confirmed! Found {num_confirmed} inverse head and shoulders pattern(s)")
    ...
    def risk_Manager(self, pattern_data: pd.DataFrame):
        if len(pattern_data) != 0:
            for x in range(0, len(pattern_data)):
                # Stop-loss 1% below the right shoulder
                pattern_data.at[x, 'stoploss'] = round(pattern_data.at[x, 'sh2_price'] * 0.99, 2)

                # Calculate head length (neckline to head distance)
                pattern_data.at[x, 'head_length'] = round(
                    pattern_data.at[x, 'neck2_price'] - pattern_data.at[x, 'head_price'], 2
                )

                # Target: head length projected above neckline
