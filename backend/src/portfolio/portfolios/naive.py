...
class NaivePortfolio(Portfolio):
    ...
    def update_positions_from_fill(self, fill: ExecutionEvent) -> None:
        """
        Takes an ExecutionEvent object and updates the position matrix
        to reflect the new position.

        Parameters:
        fill - The ExecutionEvent object to update the positions with.
        """
    ...
    def update_holdings_from_fill(self, fill: ExecutionEvent) -> None:
        """
        Takes an ExecutionEvent object and updates the holdings matrix
        to reflect the holdings value.

        Parameters:
        fill - The ExecutionEvent object to update the holdings with.
        """
        # Check whether the fill is a buy or sell
        fill_dir:int = 0
        if fill.direction == 'BUY':
        ...
        cost = fill_dir * fill_cost * fill.filled_quantity
    ...
    def update_fill(self, event: ExecutionEvent) -> None:
        ...
        """
        if event.type == 'EXECUTION' and event.status == 'FILLED':
